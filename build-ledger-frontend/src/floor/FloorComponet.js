...
const FloorComponent = ({ floor, projectTitle }) => {
    const navigate = useNavigate();

    const handleViewDetails = () => {
        navigate(`/floor/information/${floor.id}`, { state: { projectTitle } });
    };

    const handleAddInformation = () => {
        // Логика за добавяне на апартамент
        alert(`Adding apartment with ID ${floor.id}`);
    };
    ...
    return (
        <div className="project-card" style={{ cursor: 'pointer', position: 'relative', padding: '10px', border: '1px solid #ccc', borderRadius: '5px', marginBottom: '10px' }}>
            <div /*апартацтионсцлассснаме*/ style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div className="project-info" onClick={handleViewDetails} style={{ flexGrow: 1 }}>
                    <h2 style={{ margin: 0 }}>Floor Nº {floor.number}</h2>
                    <p style={{ margin: '5px 0' }}><strong>Area: </strong> {floor.area} m²</p>
                </div>
